...
class Action(metaclass=_AutoRegisterAction):
    ...
    def is_completed(self):
        return (
            (self._percent == 100 or self._state is ACTION_SUCCEEDED)
            or (self._state is ACTION_FAILED)
            or (self.state is ACTION_EXCEPTION)
            or (self.state is ACTION_REJECTED)
        )
    ...
    def _changeto_state(self, state):
        ...
        if state != self._state:
            ...
            self._state = state
            logger.info(
                "Action, name:{0} _changeto_state from {1} "
    ...
    def found_proto(self, proto):
        if (
            proto.cmdset == self._action_proto_cls._cmdset
    ...
    def found_action(self, proto):
        if (
            proto.cmdset == self._push_proto_cls._cmdset
...
class TextAction(Action):
    ...
    def __repr__(self):
        return "<action, name:{0}, state:{1}".format(
            self.__class__.__name__, self._state
        )

    def _update_action_state(self, proto_state):
        logger.debug(
            "TextAction: _update_action_state, proto_state {0}".format(proto_state)
        )
        if proto_state == "ok":
            self._changeto_state(ACTION_SUCCEEDED)
    ...
    def found_action(self, proto):
        if (
            proto._action_state == "ok"
            or proto._action_state == "error"
            or proto._action_state == "out of range"
            or proto._action_state == "error No valid marker"
            or re.match(r"Re\d{4} ok", proto._action_state)
        ):
            return True
        else:
            return False
...
class ActionDispatcher(object):
    ...
    def _on_recv(cls, self, msg):
        logger.debug(
            "ActionDispatcher: on_recv, in_progress:{0}".format(self._in_progress)
        )
        proto = msg.get_proto()
        if proto is None:
            return
        ...
        if found_action:
            if isinstance(action, TextAction):
                logger.debug(
                    "ActionDispatcher, found text action, and will update_from_push action:{0}".format(
                        action
                    )
                )
            ...
            if proto._action_id == action._action_id:
                logger.debug(
                    "ActionDispatcher, found action, and will update_from_push action:{0}".format(
                        action
                    )
                )
                if action.is_running:
        ...
        if self.has_in_progress_actions:
            ...
            for k in self._in_progress:
                ...
                if action.target == act.target:
                    ...
                    logger.error(
                        "Robot is already performing {0} action(s) {1}".format(
    ...
    def _on_action_state_changed(cls, self, action, orgin, target):
        if action.is_completed:
            ...
            if action_key in self._in_progress.keys():
                ...
                logger.warning(
                    "ActionDispatcher, del failed, action: {0}".format(action)
                )
            self._in_progress_mutex.release()
