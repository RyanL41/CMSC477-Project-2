...
class VelocityControl:
    ...
    def pw(self, detection):
        ...
        R_tag = np.array(
            [[np.cos(tag_yaw), -np.sin(tag_yaw)], [np.sin(tag_yaw), np.cos(tag_yaw)]]
        )
        robot_world = tag_world_pos + R_tag.dot(offset)

        # Adjust the robot's world yaw
        robot_world_yaw = (
            tag_yaw  # Since we already corrected offset, this simplifies orientation
        )
    ...
    def vdw(self):
        ...
        time_step = 5
        return (
            self.positions[min(int(self.timer) + 1, len(self.positions) - 1)]
            - self.positions[min(int(self.timer), len(self.positions) - 1)]
        ) / time_step
    ...
    def run(self):
        while True:
            ...
            if self.curr_state == DjikstraStates.LOOKING_FOR_START_TAG:
                ...
                for i in detections:
                    self.write_to_log(
                        f"Tagid:{self.get_apriltag_id(i)} Norm {np.linalg.norm((self.pdw() - self.pw(i)))} PDW {self.pdw()}"
                    )
                if len(detections) > 0:
                    useful_detections = [
                        d
                        for d in detections
                        if np.linalg.norm((self.pdw() - self.pw(d))) < 2
                    ]
                ...
                for i in detections:
                    ...
                    if april_tag_id == 32:
                        ...
                        if (t_ca[0] > 0.05 or t_ca[0] < -0.05) or (
                            t_ca[2] > 0.53 or t_ca[2] < 0.49
                        ):
                            self.x_vel = vel_x
                            self.y_vel = vel_y
                        else:
                ...
                if detection:
                    ...
                    self.write_to_log(
                        f"vw: {vw}, pdw: {self.pdw()}, pw: {self.pw(detection)}, vdw: {self.vdw()}"
                    )
                    ...
                    curr_rot = (
                        self.tag_id_map_rot[self.id_of_last_detection][2]
                        * (np.pi / 180)
                        + z_rot
                    )
                    ...
                    self.y_vel = vw[0]
                    self.write_to_log(
                        f"x_vel: {self.x_vel}, y_vel: {self.y_vel}, curr_rot: {np.rad2deg(curr_rot)}"
                    )
