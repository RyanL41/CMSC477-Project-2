        ...
        for _ in range(500):  # limit the number of nodes processed per frame
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    ...
                    if (
                        i == 0 and j == 0
                    ):  # or (i == -1 and j == -1) or (i == 1 and j == 1) or (i == -1 and j == 1)or (i == 1 and j == -1):
                        continue
                    if (
                        curr[0] + i >= 0
                        and curr[0] + i < 100
                        ...
                        if array[curr[0] + i][curr[1] + j] == 1:
                            ...
                            if (
                                ...
                                if (
                                    curr_w + np.sqrt(2)
                                    < weight[curr[0] + i][curr[1] + j]
                                ):
                                    prev[curr[0] + i][curr[1] + j] = (curr[0], curr[1])
                                    weight[curr[0] + i][curr[1] + j] = curr_w + np.sqrt(
                                        2
                                    )
                                else:
                                    continue
...
def path_generate():
    current = end
    path = []
    while (current[0] != start[0] or current[1] != start[1]) and (
        current[0] != -1 and current[1] != -1
    ):
        # print(curr)
        path.append(current)
        # print(current)
